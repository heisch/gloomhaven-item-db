...
const ConfirmClassDelete = () => {
  ...
  const removeItems = () => {
    ...
    itemsOwned.forEach(itemId => {
      const index = newItemsOwnedBy[itemId].findIndex( c => c === classToRemove);
      if (index != -1) {
  ...
  return (
    <Modal size="tiny" open={classToRemove !== undefined} onClose={onClose}>
        <Modal.Header>
            Remove Class {classToRemove && <ClassIcon name={classToRemove} className="confirmDeleteIcon"/>}
        </Modal.Header>
        <Modal.Content>
          <Form>
            <Form.Group>
            ...
            { itemsOwned.length > 0 && <><Form.Group>
              <p>
              Warning: This will remove all the items assigned to this character.
              </p>
              </Form.Group>
              <Form.Group>
              <p>
                {`If you are retiring this character, they would get `}
                <span style={{color: "#0000a0", fontWeight: "bold"}}>{`${goldAmount()}g`}</span> 
                {` for their items`}
              </p>
            </Form.Group></>}
          </Form>
