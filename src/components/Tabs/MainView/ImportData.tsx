...
const getImportHash = () => {
  const importHash = window.location.hash && window.location.hash.substr(1);
...
export const getShareHash = (lockSpoilerPanel: boolean) => {
  ...
  Object.values(GameType).forEach((gt: GameType) => {
    const gameStorageString = localStorage.getItem(LOCAL_STORAGE_PREFIX + gt);
...
const ImportData = () => {
  ...
  }, [dataDirty, lockSpoilerPanel, remoteData, setDataDirty, setDataMismatch]);
